...
// =========================================
// INICIALIZACIÓN (SECCIÓN CORREGIDA Y MEJORADA)
// =========================================
...
    }
    
    // === MANEJO DEL REGISTRO (sin cambios) ===
    const registerForm = document.getElementById('registerForm');
    if (registerForm) {
        registerForm.addEventListener('submit', handleRegister);
    }
    
    // === MANEJO DE LOGOUT (CORREGIDO) ===
    // 1. Definimos la variable que faltaba y buscamos las dos clases
...
    
    // === MOSTRAR DATOS DE USUARIO (ACTUALIZADO) ===
    // Comprobamos si el usuario está en una página protegida
    if (isAuthenticated()) { 
        const userData = getUserData();
        
        // 2. Actualizar el nombre
        // (Tu código original ya hacía esto, estaba perfecto)
        const userNameElements = document.querySelectorAll('.user-name');
        userNameElements.forEach(element => {
            element.textContent = userData.name;
        });

        // 3. (NUEVO) Actualizar el rol/tipo
        const userTypeElements = document.querySelectorAll('.user-type-display');
        userTypeElements.forEach(element => {
            // Capitaliza la primera letra: "gerente" -> "Gerente"
            element.textContent = userData.type.charAt(0).toUpperCase() + userData.type.slice(1);
        });
    }
    
    // === REDIRECCIONAR SI YA ESTÁ AUTENTICADO (sin cambios) ===
...
// =========================================
// EXPORTAR FUNCIONES GLOBALES (sin cambios)
// =========================================
window.authUtils = {
saveUserData,
getUserData,
